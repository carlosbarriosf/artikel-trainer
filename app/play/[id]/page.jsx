...
const Play = () => {
    ...
    const handleAnswer = () => {
        ...
        } else if (values.article !== shuffledWordArray[currentWord].article && 
            values.plural !== shuffledWordArray[currentWord].plural) {
  ...
  return (
    <section className='h-5/6 flex flex-col justify-center'>
        {listObject && 
        <>
            <h1 className='text-center text-indigo-500 font-bold'>{listObject.list.name}</h1>
            <div className='gameBoard'>
                {!gameStarted ? 
                    ...
                    <div className='h-full'>
                        <h2 className='text-center text-indigo-500 font-bold py-4'>Your Results!</h2>
                        <div className='text-sm sm:text-base flex flex-col justify-center gap-4 mt-8 px-2 text-indigo-600'>
                            <p>Correct answers: <span className='text-green-600 text-md font-bold'>{score.correct}</span></p>
                            <p>Partially correct answers: <span className='text-amber-500 text-md font-bold'>{score.half}</span></p>
                            <p>Wrong answers: <span className='text-red-500 text-md font-bold'>{score.wrong}</span></p>
                        </div>
                        <div className='flex w-full gap-4 mt-6 justify-center'>
                            <Link href='/discover-lists' className='btn bg-cyan-600 hover:bg-cyan-500 transition-all'>Try other lists!</Link>
                            <Link href='/myprofile' className='btn bg-cyan-600 hover:bg-cyan-500 transition-all'>My Profile</Link>
                        </div>
                    </div>
                    :
                    <div className='flex flex-col justify-center h-full gap-4'>
                        <h2 className='text-center text-indigo-600 font-bold py-2 text-xl'>{shuffledWordArray[currentWord].noun}</h2>
                        <form onSubmit={(e) => {
                            e.preventDefault()
                            handleAnswer()
                            if(inputRef.current) {
            ...
            {!gameEnded && 
                <div className='flex w-full justify-center gap-8 text-base'>
                    <div className='flex items-center justify-center gap-1'>
                        <FaCheckCircle size={20} color='#16a34a'/> <span>{score.correct}</span>
                    </div>
                    <div className='flex items-center justify-center gap-1'>
                        <IoIosWarning size={23} color='#f59e0b'/> <span>{score.half}</span>
                    </div>
                    <div className='flex items-center justify-center gap-1'>
                        <TiDelete size={28} color='#dc2626'/> <span>{score.wrong}</span>
                    </div>
                </div>
            }
        </>
